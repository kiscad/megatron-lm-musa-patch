...
def convert_checkpoint_from_megatron_to_transformers(mgmodel, hfmodel, args):
    ...
    for hfblock, mgblock in zip(hfvision.blocks, mgvision.decoder.layers):
        ...
        copied_numel += safe_copy(
            mgblock.input_layernorm.bias, hfblock.norm1.bias
        )
...
def convert_checkpoint_from_transformers_to_megatron(hfmodel, mgmodel, args):
    ...
    for hfblock, mgblock in zip(hfvision.blocks, mgvision.decoder.layers):
        ...
        copied_numel += safe_copy(
            ...
            hfblock.norm1.weight, mgblock.input_layernorm.weight
        )
        copied_numel += safe_copy(
            hfblock.norm1.bias, mgblock.input_layernorm.bias
        )
        # norm2 --> mlp.linear_fc1.norm
        copied_numel += safe_copy(hfblock.norm2.weight, mgblock.pre_mlp_layernorm.weight)
        copied_numel += safe_copy(hfblock.norm2.bias, mgblock.pre_mlp_layernorm.bias)
...
@torch.inference_mode()
def compare_transformers_to_megatron(hfmodel, mgmodel, args):
    if args.fp16:
        mgmodel = mgmodel.half()
        hfmodel = hfmodel.half()
    elif args.bf16:
    ...
    copied_numel = 0
    # rotary_pos_emb.inv_freq is buffer not parameter
    print(torch.allclose(hfvision.rotary_pos_emb.inv_freq, mgvision.rotary_pos_emb.inv_freq, rtol=1e-5, atol=1e-8))
    print(torch.allclose(hfvision.patch_embed.proj.weight, mgvision.patch_embed.proj.weight, rtol=1e-5, atol=1e-8))
    print(torch.allclose(hfvision.patch_embed.proj.bias, mgvision.patch_embed.proj.bias, rtol=1e-5, atol=1e-8))
    print(torch.allclose(hfvision.pos_embed.weight, mgvision.pos_embed.weight, rtol=1e-5, atol=1e-8))

    for hfblock, mgblock in zip(hfvision.blocks, mgvision.decoder.layers):
        # norm1 --> linear_qkv.norm
        print(torch.allclose(
            # hfblock.norm1.weight, mgblock.self_attention.linear_qkv.layer_norm_weight
            hfblock.norm1.weight, mgblock.pre_mlp_layernorm.weight, rtol=1e-5, atol=1e-8
        ))
        print(torch.allclose(
            hfblock.norm1.bias, mgblock.pre_mlp_layernorm.bias, rtol=1e-5, atol=1e-8
        ))
        # norm2 --> mlp.linear_fc1.norm
        print(torch.allclose(hfblock.norm2.weight, mgblock.input_layernorm.weight, rtol=1e-5, atol=1e-8))
        print(torch.allclose(hfblock.norm2.bias, mgblock.input_layernorm.bias, rtol=1e-5, atol=1e-8))
        # qkv --> self_attention.linear_qkv
        converted_weight = (
            hfblock.attn.qkv.weight.view(
                3, vision_num_query_groups, -1, vision_head_dim, vision_hidden_size
        ...
        print(torch.allclose(converted_weight, mgblock.self_attention.linear_qkv.weight, rtol=1e-5, atol=1e-8))
        converted_bias = (
            hfblock.attn.qkv.bias.view(3, vision_num_query_groups, -1)
            .transpose(0, 1)
            .flatten(1, 2)
        ...
        print(torch.allclose(converted_bias, mgblock.self_attention.linear_qkv.bias, rtol=1e-5, atol=1e-8))
        # proj --> self_attention.linear_proj
        print(torch.allclose(
            hfblock.attn.proj.weight, mgblock.self_attention.linear_proj.weight, rtol=1e-5, atol=1e-8
        ))
        print(torch.allclose(hfblock.attn.proj.bias, mgblock.self_attention.linear_proj.bias, rtol=1e-5, atol=1e-8))
        # mlp --> mlp
        fc1_weight = hfblock.mlp.linear_fc1.weight
        fc1_bias = hfblock.mlp.linear_fc1.bias
        print(torch.allclose(fc1_weight, mgblock.mlp.linear_fc1.weight, rtol=1e-5, atol=1e-8))
        print(torch.allclose(fc1_bias, mgblock.mlp.linear_fc1.bias, rtol=1e-5, atol=1e-8))
        fc2_weight = hfblock.mlp.linear_fc2.weight
        fc2_bias = hfblock.mlp.linear_fc2.bias
        print(torch.allclose(fc2_weight, mgblock.mlp.linear_fc2.weight, rtol=1e-5, atol=1e-8))
        print(torch.allclose(fc2_bias, mgblock.mlp.linear_fc2.bias, rtol=1e-5, atol=1e-8))

    # 2. vision projector
    hfprojector = hfvision.merger
    mgprojector = mgvision.projection
    print(torch.allclose(hfprojector.norm.weight, mgvision.decoder.final_layernorm.weight, rtol=1e-5, atol=1e-8))
    print(torch.allclose(hfprojector.norm.bias, mgvision.decoder.final_layernorm.bias, rtol=1e-5, atol=1e-8))

    print(torch.allclose(hfprojector.linear_fc1.weight, mgprojector.encoder.linear_fc1.weight, rtol=1e-5, atol=1e-8))
    print(torch.allclose(hfprojector.linear_fc1.bias, mgprojector.encoder.linear_fc1.bias, rtol=1e-5, atol=1e-8))
    print(torch.allclose(hfprojector.linear_fc2.weight, mgprojector.encoder.linear_fc2.weight, rtol=1e-5, atol=1e-8))
    print(torch.allclose(hfprojector.linear_fc2.bias, mgprojector.encoder.linear_fc2.bias, rtol=1e-5, atol=1e-8))
    ...
    for hfdeepstack_merger, mgdeepstack_merger, mgdeepstack_norm in zip(
        ...
        print(torch.allclose(hfdeepstack_merger.norm.weight, mgdeepstack_norm.weight, rtol=1e-5, atol=1e-8))
        print(torch.allclose(hfdeepstack_merger.norm.bias, mgdeepstack_norm.bias, rtol=1e-5, atol=1e-8))
        print(torch.allclose(
            hfdeepstack_merger.linear_fc1.weight, mgdeepstack_merger.encoder.linear_fc1.weight, rtol=1e-5, atol=1e-8
        ))
        print(torch.allclose(
            hfdeepstack_merger.linear_fc1.bias, mgdeepstack_merger.encoder.linear_fc1.bias, rtol=1e-5, atol=1e-8
        ))
        print(torch.allclose(
            hfdeepstack_merger.linear_fc2.weight, mgdeepstack_merger.encoder.linear_fc2.weight, rtol=1e-5, atol=1e-8
        ))
        print(torch.allclose(
            hfdeepstack_merger.linear_fc2.bias, mgdeepstack_merger.encoder.linear_fc2.bias, rtol=1e-5, atol=1e-8
        ))
    # n_params = sum([t.numel() for t in hfvision.state_dict().values()])
    # assert (
    #     n_params == copied_numel
    ...
    mgllm = mgmodel.language_model
    copied_numel = 0
    print(torch.allclose(hfllm.embed_tokens.weight, mgllm.embedding.word_embeddings.weight, rtol=1e-5, atol=1e-8))
    for mglayer, hflayer in zip(mgllm.decoder.layers, hfllm.layers):
        print(torch.allclose(
            hflayer.input_layernorm.weight, mglayer.input_layernorm.weight, rtol=1e-5, atol=1e-8
        ))
        ...
        print(torch.allclose(qkv_proj, mglayer.self_attention.linear_qkv.weight, rtol=1e-5, atol=1e-8))
        print(torch.allclose(
            hflayer.self_attn.o_proj.weight, mglayer.self_attention.linear_proj.weight, rtol=1e-5, atol=1e-8
        ))
        print(torch.allclose(
            hflayer.self_attn.q_norm.weight, mglayer.self_attention.q_layernorm.weight, rtol=1e-5, atol=1e-8
        ))
        print(torch.allclose(
            hflayer.self_attn.k_norm.weight, mglayer.self_attention.k_layernorm.weight, rtol=1e-5, atol=1e-8
        ))

        fc1_weight = torch.cat([hflayer.mlp.gate_proj.weight, hflayer.mlp.up_proj.weight])
        print(torch.allclose(fc1_weight, mglayer.mlp.linear_fc1.weight, rtol=1e-5, atol=1e-8))

        print(torch.allclose(hflayer.mlp.down_proj.weight, mglayer.mlp.linear_fc2.weight, rtol=1e-5, atol=1e-8))
        print(torch.allclose(
            hflayer.post_attention_layernorm.weight, mglayer.pre_mlp_layernorm.weight, rtol=1e-5, atol=1e-8
        ))

    print(torch.allclose(hfllm.norm.weight, mgllm.decoder.final_layernorm.weight, rtol=1e-5, atol=1e-8))
    if args.untie_embeddings_and_output_weights:
        # lm_head not in hfllm
        safe_copy(hfmodel.lm_head.weight, mgllm.output_layer.weight)
